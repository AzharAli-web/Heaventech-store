...
const HomeCategories = ({ categories }: { categories: Category[] }) => {
  return (
    <div className="bg-white border border-shop_light_green/20 my-10 md:my-20 p-5 lg:p-7 rounded-md">
      <Title className="border-p pb-3 "> Popular Categories.</Title>
        ...
        {categories?.map((category) => (
          <div key={category?._id} className="bg-shop_light_bg p-5 flex items-center gap-3 group">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-sm"><span className="font-bols text-shop_dark_green">{`(${category?.productCount})`}</span>items Available</p>
            </div>
          </div>
        ))}
      </div>
